...
public class RegularServiceImpl implements RegularService {
	...
	// 스케줄러 메서드는 언제 실행할 것인지에 대한 설정이 추가로 필요
	...
	// @Scheduled(cron = "0 30 9,18 * * *") // 출근할 때(09:30), 퇴근할 때(18:30) 한 번씩
	// @Scheduled(cron = "0 30 9-18 * * *") // 출근할 때부터(09:30), 퇴근할 때까지(18:30) 1시간마다 한 번씩
	// @Scheduled(cron = "0 30 9-18 * * 1-5") // 월~금까지 근무시간 1시간마다
	// @Scheduled(cron = "0 30 9-18 * * mon-fri") // 월~금까지 근무시간 1시간마다
	// @Scheduled(cron = "* * 3* * * 1") // 월요일만 1초마다
